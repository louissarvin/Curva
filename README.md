...
| **Voice-clone Play Sample chips** | After Save on the enrollment modal, 6 language chips (EN/IT/ES/FR/DE/PT per Chatterbox allowlist) pipe a fixed goal-announcement sentence through `curva.voiceClone.speak(text, locale)`. Bare returns Float32 PCM + sampleRate; renderer builds a WebAudio `AudioBuffer` and plays back immediately. Post-enrollment "aha, my voice just spoke Italian" moment. | [`VoiceEnrollmentModal.js`](https://github.com/louissarvin/Curva/blob/main/pear-app/renderer/components/VoiceEnrollmentModal.js) |
| **F4 x402 VIP room reservation (end-to-end, on-chain)** | Peer signs EIP-3009 for 5 USDT off-chain, backend facilitator settles on Sepolia, Prisma persists with `@unique slug` + `@unique txHash`. RoomBrowser shows a clickable **View on Etherscan** link on success. **Two real on-chain proofs**: [`kings-lounge` tx](https://sepolia.etherscan.io/tx/0x309447f5ec6e103a106e15bc313e4e9f9434e29512194df9adab639ccc96f19d) and [`torino-fc` tx](https://sepolia.etherscan.io/tx/0x18af4e318ea55b3796753e4bf729769f6cae27d7f693629d7c6709067a080cae). Both fire `AuthorizationUsed` + `Transfer` on the Curva USDT contract | [`RoomBrowser.js`](https://github.com/louissarvin/Curva/blob/main/pear-app/renderer/components/RoomBrowser.js), [`workers/main.js`](https://github.com/louissarvin/Curva/blob/main/pear-app/workers/main.js), [`preload.js`](https://github.com/louissarvin/Curva/blob/main/pear-app/electron/preload.js) |
| **Auto-fund on demo boot** | `scripts/demo-boot-peers.sh` waits for both peers `wallet ready`, greps the four addresses (2 owner + 2 smart), fires `scripts/fund-peers.ts` with 100 USDT each. Required because `bare/wallet/worklet.js:695-702` intentionally rotates the wallet seed each boot; without this the very first tip / VIP reservation would revert `ERC20InsufficientBalance` | [`demo-boot-peers.sh`](https://github.com/louissarvin/Curva/blob/main/pear-app/scripts/demo-boot-peers.sh) |
...
| **x402 VIP room reservation (F4, semifinal)** | [`backend/src/routes/vipRoutes.ts`](https://github.com/louissarvin/Curva/blob/a14f2e8/backend/src/routes/vipRoutes.ts) + [`backend/src/lib/vip/store.ts`](https://github.com/louissarvin/Curva/blob/a14f2e8/backend/src/lib/vip/store.ts) + [`pear-app/bare/x402Client.js`](https://github.com/louissarvin/Curva/blob/a14f2e8/pear-app/bare/x402Client.js) | Second x402 paid-resource route on the same helpers as premium translations. Peer POSTs to `/vip/reserve` with no `X-Payment` header → 402 challenge with canonical `{x402Version, accepts:[...]}` body scoped to the slug. Peer signs EIP-3009 authorization for 5 USDT to the sponsor address, retries with `X-Payment` header. Backend verifies + settles via facilitator → writes reservation to a Prisma `VipReservation` model with `@unique slug`, `@unique txHash`, `@@index(ownerAddress)`. **Fast-fail on already-reserved slug BEFORE issuing 402** (prevents wasted gas). P2002 unique-index collision handling: if two peers settle simultaneously, one gets 200, the other gets 409 with `paidTxHash` echoed for off-band escalation. Idempotent replay on same `txHash`. `GET /vip/status/:slug` is public typeahead, rate-limited. Client-side `reserveVipSlug({baseUrl, slug, wallet})` in `bare/x402Client.js` does the full 402 → sign → retry handshake. `workers/main.js` adds `evaluateVipJoinGate(slug)` at the `room:join` IPC boundary that fails open on backend outage (reservation is UX signal + directory hint, not a P2P access control; on-chain reservation is the trust root). Flag `ENABLE_VIP_RESERVATIONS`. | Peer reserves `vip-kings-lounge` on-chain by signing a single EIP-3009 authorization for 5 USDT; every other peer joining that slug reads the owner address before opening the swarm topic |
...
| **`sdk.cancel({requestId})` barge-in + user cancel** | `voiceCoach.cancelInFlight()` + `askTheFrame.cancel()` | Every `completion()` call captures `run.requestId` (per [`completion-event.d.ts:217`](https://github.com/louissarvin/Curva/blob/5d74a90/pear-app/bare/voiceCoach.js#L233-L248)). Starting a new voice-coach turn auto-cancels the previous in-flight completion (barge-in). Ask-the-frame overlay has a Cancel button that fires as soon as `voice:answer-token` starts streaming. `close()` cancels before teardown so completions never leak past a room switch. Client-side cancels a pre-requestId race with `{ok:false, code:'NO_INFLIGHT'}` because `cancel.d.ts:47` requires `z.ZodString.min(1)`. | Hold Space to say "actually never mind", release, hold Space again for a new question — the previous stream cancels cleanly. No dangling completion, no zombie tokens. |
| **`@qvac/diagnostics` native diagnostic report** | `@qvac/diagnostics@0.1.2` via [`bare/diagnosticsReport.js`](https://github.com/louissarvin/Curva/blob/5d74a90/pear-app/bare/diagnosticsReport.js) | Wraps `generateReport({app})` + `serializeReport(report)`. Registers a `curva-room` extension section carrying `{roomSlug, isHost, activeCapabilities}` before generating so the report includes room context. Dynamic import so a missing package is a graceful no-op. Surface validation for the five methods actually used before setting `available=true` (a bare `{}` fake would slip past a truthy check). Fourth "Report" tab in DiagnosticsPanel exposes Generate + Copy to clipboard, `<pre>` populated via `textContent` only. | Judges click "Generate report" and get a full `DiagnosticReport` covering AppInfo + EnvironmentInfo + HardwareInfo + Curva room extension — one click, Tether-native library, no `console.log` hacks |
| **`sdk.loggingStream({id})` per-model tail with concurrent multiplex** | `startModelLogTail(sdk, {modelId, onLog, maxLines})` in [`bare/observability.js`](https://github.com/louissarvin/Curva/blob/6adf1ff/pear-app/bare/observability.js) | Every row in the DiagnosticsPanel Models tab gets a "Tail logs" button that opens an inline drawer with a live `sdk.loggingStream({id: modelId})` iterator. Filter chips (`all \| error \| warn \| info \| debug`) are client-side so they never stop the stream. 200-line drop-oldest buffer with `HH:MM:SS.mmm` timestamps. Only one drawer open at a time. **Multiplex behaviour**: a synchronous second call with the same `modelId` joins the shared upstream via a module-level `Map` rather than re-opening the RPC — each caller has its own `stopped` flag and the upstream terminates only when the last caller stops. Prevents duplicate SDK subscriptions on a fast Refresh double-click. `SDK_LOG_ID = '__sdk__'` verified at `node_modules/@qvac/sdk/dist/logging/namespaces.d.ts:1`. | Click "Tail logs" on the Whisper row: watch every audio frame log arrive live, filter to `debug` to see the VAD decisions, hit Stop or switch tabs to close the stream cleanly. Model-scoped observability without opening a shell. |
| **Voice-cloned goal pipeline (F1)** | Chatterbox routing in [`bare/goalPipeline.js`](https://github.com/louissarvin/Curva/blob/a14f2e8/pear-app/bare/goalPipeline.js) | `routeTts()` helper: when the host has enrolled a Chatterbox reference AND the target locale is in the widened allowlist (EN/IT/ES/FR/DE/PT verified against `node_modules/@qvac/sdk/dist/schemas/text-to-speech.d.ts:2` `TTS_CHATTERBOX_LANGUAGES`), goal announcements speak via `voiceClone.speakStream(text, locale)`. Falls back to Supertonic `announcer.openSpeakStream({locale})` when reference not enrolled or locale unsupported. Never fails the pipeline on TTS routing errors. Emits `via: 'voiceClone' \| 'announcer'` field on events for observability. Flag `CURVA_VOICE_CLONE_GOAL_ENABLED`. | Enroll a 6-second sample of your voice on Peer A; when the goal fires, every peer hears the announcement in your cloned voice, in their own language |
| **Voice coach conversational memory (F2)** | Session-scoped ring buffer in [`bare/voiceCoach.js`](https://github.com/louissarvin/Curva/blob/a14f2e8/pear-app/bare/voiceCoach.js) | 6-turn ring of `{userText, coachAnswer, at}` prepended between the sanitised system prompt and the current user turn. Retrieved RAG snippets STILL flow through `<retrieved_untrusted>` fencing before the LLM sees them — memory turns are trusted-by-construction (came from OUR LLM plus the user's own already-sanitised transcript). Auto-cleared on `close()` and on room-slug change so switching rooms starts cold. Preload bridge `curva.voiceCoach.clearMemory()` powers a "Reset conversation" link in the VoiceCoachPanel header. Flag `CURVA_VOICE_COACH_MEMORY_ENABLED` defaults ON. | Ask "who is the top scorer?" then "and their assist count?" — coach connects the dots without you repeating the player name |
| **Match recap synthesis (7-cap flow, F3)** | `chat.history` + `sharedLlmHandle.completion` + `translate` + Chatterbox / Supertonic + Hyperblob persist in [`bare/matchRecap.js`](https://github.com/louissarvin/Curva/blob/a14f2e8/pear-app/bare/matchRecap.js) | Reads last 200 chat rows from Autobase, filters into buckets (goal cards, tips, prediction outcomes, user chat), sanitises with NFKC + control-strip + suspicious-prefix drop, feeds through a fenced `<retrieved_untrusted>` block into Qwen3 with `reasoning_budget:0` for a <80-word recap. Per configured locale, translates via Bergamot, routes TTS through Chatterbox if enrolled else Supertonic, saves the audio as a Hyperblob attached to the room's Hyperdrive, then broadcasts a `system:match-recap` chat message with `{recapText, audioByLocale: {locale: blobKey}}`. 60s end-to-end timeout, `BUSY` guard against concurrent generates. Flag `CURVA_MATCH_RECAP_ENABLED` (default OFF, heavy). | Full-time: click Generate Recap. A 60-word summary lands in chat with a Play button per locale that pulls the blob and speaks it in your voice, in your friend's language |
| **Voice-cloned commentator (F5)** | Chatterbox routing in [`bare/commentator.js`](https://github.com/louissarvin/Curva/blob/a14f2e8/pear-app/bare/commentator.js) | Symmetric to F1 for the running commentary: `routeTts()` helper wraps a buffering session that accumulates streamed tokens, calls `voiceClone.speakStream(fullText, locale)` on turn end, and adapts its chunks iterator to the announcer's `{write, end, destroy, chunks}` shape so the token loop drives both paths uniformly without blocking on TTS. Same allowlist as F1. Never fails the commentator loop on TTS routing errors — catch, log, fall through to announcer. Flag `CURVA_COMMENTATOR_VOICE_CLONE_ENABLED`. | Continuous voice presence throughout the match: every kick, every save, every shot narrated in your cloned voice |
| **Semantic room chat search (F6)** | Per-room RAG workspace in [`bare/roomSearch.js`](https://github.com/louissarvin/Curva/blob/a14f2e8/pear-app/bare/roomSearch.js) | Workspace id `curva-room-search-<sha256(roomSlug).slice(0,16)>` deliberately isolated from the `curva/room/<slug>/chat` glossary namespace `bare/rag.js` uses so roomBot glossary ingests never pollute chat hits. On construction, async `reindexAll()` pulls `chat.history({from:0, limit:500})`, chunks per-message via `sdk.ragChunk({text, chunkSize:200, chunkOverlap:20})`, ingests with deterministic per-chunk ids for idempotent reingest. `chat.js` untouched — the existing `onMessage(cb)` subscription IS the docs-verified message-applied hook (fires post-apply in `emitNew` loop). NFKC + control-strip on every ingested text AND every query. 15s debounce on additional reindex passes. Persists across boots (no `deleteOnClose`). Search bar in Chat panel is debounced 250ms; results overlay under the input. Flag `CURVA_ROOM_SEARCH_ENABLED` defaults ON. | Type "penalty" in the search bar: live semantic hits over 200+ messages in <200ms, all on-device, click a hit to scroll + pulse-highlight |
| **Auto-highlight detection (5-cap flow, F7)** | MobileNetV3 + SmolVLM2 + Qwen3 + Bergamot + TTS in [`bare/highlightPipeline.js`](https://github.com/louissarvin/Curva/blob/a14f2e8/pear-app/bare/highlightPipeline.js) | Same pattern as the goal pipeline extended to red cards, yellow cards, corner kicks, substitutions. MobileNetV3 pre-filter (0.3 min confidence, top-3, fail-open on classifier error) → SmolVLM2 multi-choice verify with strict regex parse `^([A-E])(\s+(HOME\|AWAY\|[A-Z]{3}))?$` → Qwen3 summariser with deterministic fallback → 30s debounce per `{kind, team}` pair with 16-entry recent-highlights ring. 15s per-tick timeout budget. Refuses to hallucinate: non-match regex or 'E' (none) returns null. Per-locale translate + `routeTts()` fanout (voiceClone or announcer). Emits `system:highlight` chat message with `{kind, team, summaryText, audioByLocale}`. Chat renderer shows a colored pill (red/yellow/blue/green variants). Flag `CURVA_AUTO_HIGHLIGHT_ENABLED` (default OFF, heavy). | Match plays; a highlight card appears in chat every time the VLM catches a card / corner / sub, in every viewer's language |
| **RAG-augmented commentator (F9)** | `rag.search` race in [`bare/commentator.js`](https://github.com/louissarvin/Curva/blob/a14f2e8/pear-app/bare/commentator.js) | Every trigger runs `enrichPromptWithRag(rag, pulseContext)` that races `rag.search({query, topK:3})` against `RAG_SEARCH_TIMEOUT_MS=800`. Query built from `buildRagQuery({matchTitle, matchTimeSeconds, triggerType, currentScore})` — short + fact-shaped for a targeted vector. On success: sanitise each snippet (NFKC + control-strip + `SNIPPET_PREFIX_DENYLIST` reject on `ignore previous`, `system:`, `assistant:`, `user:`, `<\|system\|>`, `###`, `<system>`), cap at 300 chars, wrap in `<retrieved_untrusted>` block placed BEFORE the persona system message. On timeout / missing handle / thrown / bad shape: degrade to no-retrieval commentary (`degraded: 'TIMEOUT' \| 'NO_HANDLE' \| 'THREW' \| 'BAD_SHAPE'`). Emits `commentator:rag-injected {snippetCount, searchedQuery}` for observability. Flag `CURVA_COMMENTATOR_RAG_ENABLED` (default OFF; opt-in for demo because the 800ms budget stacks on the tick envelope). | Commentary shifts from generic ("great save!") to informed ("Messi's 4th of the tournament, matches Mbappé for the golden boot lead") |
| **QVAC asset seed-back mesh (`sdk.downloadAsset` + Hyperdrive, F13)** | [`bare/qvacAssetSeed.js`](https://github.com/louissarvin/Curva/blob/a14f2e8/pear-app/bare/qvacAssetSeed.js) — marquee QVAC × Pears composition | Per-peer Hyperdrive under `corestore.namespace('curva/qvac-assets')` with manifest at `/manifest.json` and assets at `/assets/<assetId>.bin`. Manifest hydrated + defensively cleaned on boot (drops entries with bad shape or invalid assetIds). Resolution ordering: (1) local manifest → loopback `hypercore-blob-server` URL, (2) DHT client-join `sha256('curva:qvac-asset:' + assetId)` with 3s cold-start budget, (3) null → caller invokes `downloadAndSeed`. `downloadAndSeed(assetId, registryUrl)` calls `sdk.downloadAsset({assetSrc, seed:true, onProgress})` — the SDK's own hyperdrive routing kicks in when `seed:true`. Emits `qvac-asset:seeded {assetId, blobKey, sizeBytes}`. Security: assetId regex `^[a-zA-Z0-9-]{1,64}$` blocks path traversal + hash-input bloat; registryUrl restricted to `http:`/`https:`/`pear:`; `MAX_ASSET_BYTES = 512 MiB`; loopback + token URLs only. Flag `CURVA_QVAC_ASSET_SEED_ENABLED` (default OFF). | Peer A downloads a Bergamot pair once. Peer B discovers Peer A via the asset DHT topic and pulls from Peer A's Hyperdrive instead of a central mirror. True P2P model distribution |
| **4 new MCP tools for the voice coach (F14)** | [`backend/src/lib/mcp/tools.ts`](https://github.com/louissarvin/Curva/blob/a14f2e8/backend/src/lib/mcp/tools.ts) | `get_prediction_pool {roomSlug}` reads open Prisma pools; `get_user_profile {ownerAddress}` returns aggregate stats via parallel Prisma counts (isDemo excluded); `get_h2h_history {teamA, teamB}` reads `backend/src/data/world-cup-2026.json` (fails gracefully to empty when data source missing); `get_tournament_bracket {competition}` returns `{r16, qf, sf, third_place, final}` from WC26 corpus. All read-only, Prisma parameterized, bounded caps (20 rows, 256 chars). `loadWcCorpus()` caches once + returns null on load failure — every tool degrades to `available:false` rather than throwing. Registered alongside the existing 12 tools in the same registration block (16 total). Voice coach discovers all tools dynamically via MCP `tools/list` — no client-side wiring needed. | Ask the voice coach "who won last time these teams played?" — coach chains STT → RAG → LLM → MCP `get_h2h_history` → Supertonic in one turn |
| **Multi-locale commentator fanout (F16)** | `Promise.allSettled` per-locale in [`bare/commentator.js`](https://github.com/louissarvin/Curva/blob/a14f2e8/pear-app/bare/commentator.js) | Symmetric to the F1 goal pipeline fanout. After LLM emits sanitised commentary in the source locale, translate the same sentence per additional locale via `translateWithTimeout` (1500ms per locale, skip + log on timeout) and route each translated line through `routeTts()` in parallel via `Promise.allSettled`. Slow Bergamot pair on one locale never blocks fast ones. Locale bypass: skips fanout when `locales === [sourceLocale]`. Emits `commentator:multi-locale-speak {locale, source, via}` per locale for observability. Flag `CURVA_COMMENTATOR_MULTI_LOCALE_ENABLED` (default OFF, heavy: extra Bergamot + N TTS opens per pulse). | Room has EN, IT, ID viewers configured. On every pulse, three locales speak in parallel; nobody waits for the slowest translation |
| **Cross-lingual voice coach polish (F22)** | Bergamot bracket in [`bare/voiceCoach.js`](https://github.com/louissarvin/Curva/blob/a14f2e8/pear-app/bare/voiceCoach.js) | When STT-classified user locale ≠ `en` AND translate handle is available: translate transcript ID/ES/FR/... → EN via `translate.translate(transcript, 'en')` with 500ms timeout + raw fallback, use the translated query for `rag.search(...)` (better retrieval quality against the English FIFA corpus vs mixed-language embeddings), LLM prompt keeps the ORIGINAL transcript as the user turn (Qwen3 handles cross-lingual natively), then translate the LLM answer EN → user locale with 500ms timeout + raw fallback before TTS synthesis. Preserves existing behaviour when detected locale === 'en' (byte-identical). Emits `voice:cross-lingual {userLocale, translatedQueryToEn, translatedAnswerBack}` when the bracket fires. Flag `CURVA_VOICE_COACH_CROSS_LINGUAL_ENABLED` defaults ON (quality upgrade, no downside). | Indonesian user asks the coach a question; coach retrieves against the English corpus, answers in Indonesian |

Cross-capability flows are the biggest signal that Curva actually reasoned about how to combine QVAC primitives rather than just calling each in isolation. The three baseline flows (voice-coach 5-cap, ask-the-frame 5-cap, goal-pipeline 6-cap) sit alongside eight more this session shipped: voice-cloned goal (F1), voice-cloned commentator (F5), match recap 7-cap (F3), auto-highlight 5-cap (F7), RAG-augmented commentator (F9), multi-locale commentator fanout (F16), cross-lingual voice coach (F22), and the marquee QVAC × Pears composition — asset seed-back mesh (F13). All covered by unit + integration tests under [`pear-app/test/`](https://github.com/louissarvin/Curva/blob/a14f2e8/pear-app/test/) that exercise the full stitching with fake SDKs.
...
- **`sdk.cancel({requestId})` barge-in** — Rejected letting the previous completion run to completion when the user starts a new voice-coach turn because on-camera it looks broken: two tokens streams overlapping. Trade-off: the SDK schema does not accept an empty `requestId`, so a cancel that races the very first begin can't be sent. We return `NO_INFLIGHT` client-side and accept that pre-requestId turns cannot be barge-in-cancelled.
- **`@qvac/diagnostics` native report** — Rejected hand-rolling a `{app, env, hardware}` JSON blob because it duplicates work the Tether-native library already does and it drifts. Trade-off: hard dep on `@qvac/diagnostics` which is currently at v0.1.2 (pre-1.0); a breaking change would land in our report shape. Mitigation: we wrap it behind `bare/diagnosticsReport.js` so the migration surface is one file.
- **`sdk.loggingStream({id})` per-model tail** — Rejected reusing the union view from `subscribeServerLogs` because a judge picking one model wants to see just that model's logs, and filtering client-side wastes a lot of RPC bandwidth on models they don't care about. Trade-off: two independent subscription surfaces (`subscribeServerLogs` for the union tail, `loggingStream` for the per-model drilldown) mean two separate cleanup paths; a leak in one does not stop the other. We accept that over a single overloaded API because the semantics are clearer.
...
- **Voice-cloned goal pipeline (F1)** — Rejected leaving all goal announcements in the stock Supertonic voice because the "your friend just yelled GOAL" moment only lands if the voice is actually theirs. Trade-off: Chatterbox has an EN/IT/ES/FR/DE/PT allowlist (widened from the initial EN/IT after re-verifying against `text-to-speech.d.ts:2`). Everything outside that set falls back to Supertonic without a warning — the user hears their goal announcement in Supertonic, not their voice. We call this out in the UI.
- **Voice coach conversational memory (F2)** — Rejected stateless per-turn coaching because follow-up questions never worked ("and their assist count?" needed the player name repeated). Trade-off: kvCache reuse gets slightly less effective (prefix changes each turn). Flag `CURVA_VOICE_COACH_MEMORY_ENABLED` lets us kill it instantly if latency regresses.
- **Match recap synthesis 7-cap (F3)** — Rejected letting the room end without a summary because the recap is the moment a distant fan gets to relive the match in their own language, in their friend's voice. Trade-off: 7-cap orchestration is the densest chain in Curva; a failure mode in any single capability (chat read, LLM completion, Bergamot pair, TTS, Hyperblob) fails the whole recap. Mitigation: 60s end-to-end timeout, `BUSY` guard against concurrent generates, `CURVA_MATCH_RECAP_ENABLED` defaults OFF because it's heavy.
- **Voice-cloned commentator (F5)** — Rejected shipping voice-clone only for the goal announcement because a single beat of your voice per goal is not enough — the continuous commentary is where the "friend narrating" feeling comes from. Trade-off: `routeTts()` needs a buffering session wrapper because `voiceClone.speakStream` wants the full text up front but the commentator streams tokens. We accept the extra latency (buffer until turn end) because the alternative is switching to `voiceClone.speak(text)` at every token boundary, which doesn't work.
- **Semantic room chat search (F6)** — Rejected linear-scan chat search because it does not scale past a few hundred messages, and the whole point of the search bar is late-joiners catching up. Trade-off: initial `reindexAll` reads up to 500 rows via `chat.history({from:0, limit:500})` on room open; each ingest runs an embed model call. Fire-and-forget, but on a large room + slow disk this could cause several seconds of embed load. Mitigated by dedupe-by-chunk-id on repeat runs + 15s debounce.
- **Auto-highlight detection 5-cap (F7)** — Rejected relying purely on the goal pipeline for match events because cards / corners / subs deserve chat surface too. Trade-off: the pipeline WILL hallucinate a highlight if the SmolVLM2 answer doesn't match the strict regex `^([A-E])(\s+(HOME\|AWAY\|[A-Z]{3}))?$` — so we refuse to publish, returning null. Same posture as goalPipeline. Flag defaults OFF because the pipeline is heavy (VLM + LLM per activity spike).
- **RAG-augmented commentator (F9)** — Rejected making the commentator query RAG on every pulse because the 800ms search budget stacks on top of the tick envelope. Trade-off: the flag is OFF by default. When ON, `enrichPromptWithRag` races against a hard 800ms deadline and degrades gracefully (`degraded: 'TIMEOUT' \| 'NO_HANDLE' \| 'THREW' \| 'BAD_SHAPE'`) to no-retrieval commentary rather than blocking. Every snippet passes through the same NFKC + prefix-denylist + `<retrieved_untrusted>` fence as the voice coach.
- **QVAC asset seed-back mesh (F13)** — Rejected letting every peer independently download the same 60 MB Bergamot pair from a central mirror because it defeats the "P2P everything" story of the Tether stack. Trade-off: `findPeerAssetUrl` currently returns null after DHT topic client-join because there is no cross-peer URL-exchange protocol yet — the renderer falls through to `downloadAndSeed` and the SDK's own hyperdrive routing kicks in when `seed:true`. Peer-to-peer redistribution IS working via the SDK; the "peer A serves the URL directly to peer B" story is code-review-visible via `resolveAsset`/`downloadAndSeed` but not full-loop demonstrable yet. Documented in-code and safe to iterate on later.
- **4 new MCP tools (F14)** — Rejected forcing the voice coach to guess prediction pools / user profile / head-to-head history from RAG hits alone because MCP tool calls are the right primitive for structured data. Trade-off: no `world-cup-2022.json` exists on disk, so `get_h2h_history` returns empty gracefully — the voice coach handles the empty case in its answer. Every tool is Prisma parameterized with 20-row caps.
- **Multi-locale commentator fanout (F16)** — Rejected letting the commentator emit in ONE locale because the goal pipeline already fans out to N locales; asymmetry was ugly and unfair to non-English viewers. Trade-off: parallel `Promise.allSettled` per additional locale means a slow Bergamot pair on one locale never blocks fast ones, but the tick budget stacks with the number of locales. Flag defaults OFF.
- **Cross-lingual voice coach (F22)** — Rejected letting non-English users get worse retrieval quality than English users because the FIFA corpus is English and per-language embedding vectors don't cross well. Trade-off: extra 500ms budget per direction (query IN, answer OUT). EN users bypass the bracket (byte-identical). Flag defaults ON because the quality upgrade is real and the fallback is graceful (raw transcript / raw answer).
- **F21 OCR audit trail** — Rejected throwing away the goal-source frame after inference because a viewer disputing a goal card ("no way that was a goal") had nothing to point at. Trade-off: saves the raw frame as a Hyperblob attached to the room's Hyperdrive before sending `system:goal-card`. Adds ~50-200 KB per goal to peer storage. Flag defaults OFF; when ON, chat renders a Verify button that opens a lightbox with the exact frame that fired the detection.
- **F20 semantic scrubber jump** — Rejected letting F6 search hits only scroll chat because the video playhead is already tracked at message time and jumping to that moment is free. Trade-off: hits without `matchTimeMs` silently skip the scrub — no `[t+MM:SS]` badge, no scrub call.
- **F4 x402 VIP room reservation** — Rejected leaving the x402 spec expressed on a single premium-translations route because "one x402 route" is a demo, "two x402 routes on the same helpers" is a pattern. Trade-off: settled-but-collision race (both peers pay for the same slug simultaneously, Prisma P2002 sends one to 409) means the losing peer's payment lands on-chain with no auto-refund. Response includes `paidTxHash` so client can escalate off-band. Documented in `SECURITY.md`.
...
- **QVAC baseline completion behavior**: `voice-coach.test.js`, `ask-the-frame.test.js`, `goal-pipeline.test.js`, `commentator.test.js`, `commentator-streaming.test.js`, `commentator-stt.test.js`, plus `test/integration/*.test.js`
- **QVAC session-shipped orchestration flows**: `voice-clone-goal-pipeline.test.js` (F1), `voice-coach-memory.test.js` (F2), `match-recap.test.js` (F3), `commentator-voice-clone.test.js` (F5), `room-search.test.js` (F6), `highlight-pipeline.test.js` (F7), `commentator-rag.test.js` (F9), `qvac-asset-seed.test.js` (F13), `commentator-multi-locale.test.js` (F16), `voice-coach-cross-lingual.test.js` (F22)
- **Session-shipped UX**: `scrubber-jump.test.js` (F20), `goal-proof.test.js` (F21), `notifications.test.js` (F8), `model-log-tail.test.js`, `qvac-cancel.test.js`, `diagnostics-report.test.js`
...
- **Prompt-injection defense**: assertions across `test/integration/`, `voice-coach.test.js`, `ask-the-frame.test.js`, `match-recap.test.js`, `commentator-rag.test.js`, `highlight-pipeline.test.js`
- **Backend**: `sharedRag.test.js`, `delegatedProvider.test.js`, `matchClipDrive.test.js`, `enhanced-tools.test.js`, `mcp/extra-tools.test.ts` (F14 four new tools), `routes/vip.test.ts` (F4 x402 VIP)

Total: ~750+ tests passing across new-feature suites (195/195 on the F1-F22 session work alone) + regression across the baseline. See `bun test` in `backend/` and `npx brittle-node test/*.test.js` in `pear-app/`.
...
- `hyperschema` typed chat encoding — retrofit cost 6-8h breaking the Autobase apply function, unacceptable risk this cycle.
...
- F13 cross-peer URL-exchange handshake — F13 asset seed-back mesh is live and the SDK's own hyperdrive routing kicks in when `seed:true`, but the explicit peer-A-serves-URL-to-peer-B handshake for `findPeerAssetUrl` is a protocol design in-flight. Currently returns null after DHT topic client-join. Documented in-code.
